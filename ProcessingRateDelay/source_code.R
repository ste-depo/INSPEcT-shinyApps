...
m_t_complete <- function( t , k , j , l ) {
  if( j != l ) {
    k / l * ( j * ( 2 - exp( -l*t ) ) -  l * ( 2 - exp( -j*t ) ) ) / ( j - l )
  } else {
    k / l * ( 2 - exp( -l*t )*( 1 + l*t ) )
  }
}

m_t_simple <- function( t , k , l ) {
  k / l * ( 2 - exp(-l*t) )
}
...
hrtime <- function(j, l, maxerr=1e-2) 
{
  ...
  relerror <- HRfullsys$objective / j
  if( relerror < maxerr )
...
plot_gene_fun <- function(k1, k2, k3, absval, metrics) {
  ...
  metric1 <- half_resp_time/half_resp_time_noproc
  metric2 <- mat_levels_noproc - mat_levels
  ...
  if( metrics ) {
    ...
    title( bquote(tau ~ '=' ~ frac("t"[1/2] ~ "'", "t"[1/2]) ~ '=' ~ 
      .(round(metric1,2)) ~ ';' ~ Delta ~ '=' ~ .(round(metric2,2)) ) )
    ...
    title( bquote(tau ~ '=' ~ .(round(metric1,2)) ~ ';' ~ 
      Delta ~ '=' ~ .(round(metric2,2)) ) )
