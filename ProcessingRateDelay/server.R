...
shinyServer(function(input, output, session) {
	...
	loadFromFile <- reactive({

		filename <- input$file1$datapath 
		if( is.null(filename) ) filename <- "mycerIds_20genes.rds"
			...
			return(NULL)
				...
				metrics <- data.frame(
					m1 = sapply(1:nrow(ids), function(i) 
						m1( rates$k2[i] , rates$k3[i] )),
					m2 = sapply(1:nrow(ids), function(i) 
						m2( rates$k1[i], rates$k2[i] , rates$k3[i] ))
					)
	...
	output$gene1 <- renderPlot({
		...
		if( length(input$k1_gene1)>0 ) {
			...
			metrics_gene1$metric1 <- out$metric1
			metrics_gene1$metric2 <- out$metric2
	...
	output$metrics_dotplot <- renderPlot({
		...
		if( !is.null(data) & length(input$k1_gene1)>0 ) {
			...
			plot(metrics$m2, metrics$m1, log='xy', pch=20, col = cols , 
				xlim = range(c(metrics$m2, metrics_gene1$metric2)), 
				ylim = range(c(metrics$m1, metrics_gene1$metric1)),
			...
			points( metrics_gene1$metric2 , 
				metrics_gene1$metric1 , cex=1.5, lwd=5, col='gray50')
